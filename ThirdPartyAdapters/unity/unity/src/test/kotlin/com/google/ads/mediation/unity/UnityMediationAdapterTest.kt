...
class UnityMediationAdapterTest {
  ...
  fun setUp() {
    unityMediationAdapter =
      UnityMediationAdapter(unityInitializer, unityAdsWrapper, unityBannerViewFactory, unityAdsLoader)
  ...
  fun collectSignals_forBannerFormatAndActivityContext_invokesSignalCallbacks() {
    whenever(unityAdsWrapper.getToken(any(), any())) doAnswer { invocation ->
  ...
  fun collectSignals_forUnsupportedFormat_invokesSignalCallbacks() {
    whenever(unityAdsWrapper.getToken(any())) doAnswer { invocation ->
