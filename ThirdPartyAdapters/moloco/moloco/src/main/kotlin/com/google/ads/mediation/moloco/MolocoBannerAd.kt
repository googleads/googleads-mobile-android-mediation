...
private constructor(
  ...
  fun loadAd(context: Context) {
    ...
    MolocoSdkFactory.delegate.createBanner(
  ...
  companion object {
     ...
     *
     * 1. `height == 0` → [BannerAdSize.InlineAdaptive]. All Google inline adaptive factory methods
     *    return `height == 0`; `isAutoHeight` covers the legacy smart-banner path. Checked first —
     *    fixed formats always carry a positive height.
     * 2. Exact dimensions → [BannerAdSize.Standard] / [BannerAdSize.MREC] / [BannerAdSize.Tablet].
     * 3. Height matches [AdSize.getCurrentOrientationAnchoredAdaptiveBannerAdSize],
     *    [AdSize.getPortraitAnchoredAdaptiveBannerAdSize] or
     *    [AdSize.getLandscapeAnchoredAdaptiveBannerAdSize] for the same width →
     *    [BannerAdSize.AnchoredAdaptive].
     * 4. Fallback → [BannerAdSize.InlineAdaptive].
     */
    internal fun resolveBannerAdSize(context: Context, adSize: AdSize): BannerAdSize =
      when {
        ...
        isAnchoredAdaptiveSize(context, adSize) ->
    ...
    /**
     * Wraps the calls into Google's [AdSize] adaptive-size APIs so that if they throw (e.g. due to
     * an unusual [Context] state), we degrade to [BannerAdSize.InlineAdaptive] instead of crashing
     * the ad load.
     */
    private fun isAnchoredAdaptiveSize(context: Context, adSize: AdSize): Boolean =
      try {
        adSize.height ==
          AdSize.getCurrentOrientationAnchoredAdaptiveBannerAdSize(context, adSize.width).height ||
          adSize.height ==
            AdSize.getPortraitAnchoredAdaptiveBannerAdSize(context, adSize.width).height ||
          adSize.height ==
            AdSize.getLandscapeAnchoredAdaptiveBannerAdSize(context, adSize.width).height
      } catch (e: Exception) {
        false
      }
  }
