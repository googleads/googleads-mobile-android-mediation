...
public class VungleMediationAdapter extends RtbAdapter implements MediationRewardedAd,
  ...
  public void collectSignals(@NonNull RtbSignalData rtbSignalData,
    ...
    VungleSdkWrapper.delegate.getBiddingToken(rtbSignalData.getContext(), new BidTokenCallback() {
      @Override
      public void onBidTokenCollected(@NonNull String bidToken) {
        Log.d(TAG, "Liftoff Monetize bidding token=" + bidToken);
        signalCallbacks.onSuccess(bidToken);
      }

      @Override
      public void onBidTokenError(@NonNull String errorMessage) {
        AdError error = new AdError(ERROR_CANNOT_GET_BID_TOKEN,
            "Liftoff Monetize returned error on collecting bid token: " + errorMessage,
            ERROR_DOMAIN);
