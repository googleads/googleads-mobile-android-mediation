...
public class PangleConstants {
   ...
   * User is a child.
   *
   * <p>Shouldn't call Pangle SDK if the user is a child. Adapter will respond with this error
   * code if adapter is requested to load ad or collect signals when the user is a child.
   *
   * <p>Starting with Pangle SDK V71, Pangle no longer supports child user flags and you may
   * not initialize or use the Pangle SDK in connection with a "child" as defined under applicable
   * laws.
   */
  ...
  public static boolean isChildUser() {
    ...
    return requestConfiguration.getTagForChildDirectedTreatment()
            ...
            || requestConfiguration.getTagForUnderAgeOfConsent() == RequestConfiguration.TAG_FOR_UNDER_AGE_OF_CONSENT_TRUE;
  }
