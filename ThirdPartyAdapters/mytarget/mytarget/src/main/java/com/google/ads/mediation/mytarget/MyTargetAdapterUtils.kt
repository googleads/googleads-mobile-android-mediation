...
object MyTargetAdapterUtils {
  ...
  fun hasACConsent(context: Context, vendorId: Int): ConsentResult {
    ...
    val gdprApplies =
      try {
        sharedPref.getInt("IABTCF_gdprApplies", -1)
    ...
    val additionalConsentString =
      try {
        sharedPref.getString("IABTCF_AddtlConsent", "") ?: ""
      } catch (exception: ClassCastException) {
        Log.w(
          TAG,
    ...
    if (version == 1) {
      ...
      if (additionalConsentParts.size == 2) {
        val consentedIds = additionalConsentParts[1].split(".").toTypedArray()
      ...
      val disclosedIds = additionalConsentParts[2].split(".").toTypedArray()
