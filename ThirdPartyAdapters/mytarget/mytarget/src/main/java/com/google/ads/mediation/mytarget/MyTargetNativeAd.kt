...
class MyTargetNativeAd(
  ...
  fun loadAd(adConfiguration: MediationNativeAdConfiguration) {
    ...
    if (slotId < 0) {
      ...
      Log.e(MyTargetInterstitialAd.TAG, slotIdError.message)
    ...
    if (adConfiguration.nativeAdOptions.shouldReturnUrlsForImageAssets()) {
  ...
  override fun trackViews(
    containerView: View,
    clickables: Map<String?, View?>,
    nonClickables: Map<String?, View?>,
  ) {
    val clickableViews: java.util.ArrayList<View?> = java.util.ArrayList<View?>(clickables.values)
    containerView.post {
  ...
  private fun findMediaAdViewPosition(
    clickableViews: MutableList<View?>,
    mediaAdView: MediaAdView,
  ): Int {
    for (i in clickableViews.indices) {
      val view = clickableViews[i]
      if (view is MediaView || view is com.google.android.gms.ads.formats.MediaView) {
        val mediaView = view
