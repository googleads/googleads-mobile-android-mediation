...
class InMobiRtbNativeAdTest {
  ...
  fun trackViews_registersClickableAssetViewsAndCallsRegisterForTracking() {
    ...
    rtbNativeAd.inMobiUnifiedNativeAdMapper.trackViews(
      containerView,
      clickableAssetViews,
      emptyMap(),
    )
    ...
    assertThat(rtbNativeAd.inMobiUnifiedNativeAdMapper.overrideClickHandling).isTrue()
  }
  ...
  fun onVideoCompleted_invokesOnVideoComplete() {
    ...
    whenever(nativeAdConfiguration.bidResponse).thenReturn("test_bid_token")
    whenever(inMobiAdFactory.createInMobiNativeWrapper(eq(context), eq(67890L), eq(rtbNativeAd)))
