...
final class AdapterInlineListener implements InlineAdView.InlineAdListener, InlineAdFactory.InlineAdFactoryListener {
	...
	AdapterInlineListener(final MediationBannerAdapter adapter, final MediationBannerListener listener, final LinearLayout internalView) {
			...
			case VASAds.ERROR_AD_REQUEST_TIMED_OUT:
				ThreadUtils.postOnUiThread(new Runnable() {
					...
					public void run() {
						...
						if ((bannerListener != null) && (adapter != null)) {
							bannerListener.onAdFailedToLoad(adapter, AdRequest.ERROR_CODE_NETWORK_ERROR);
						}
					}
				});
				break;
