...
/**
 * Calls [Adapter.loadNativeAdMapper] with the given [MediationNativeAdConfiguration] and verifies
 * [MediationAdLoadCallback.onFailure] with the expected [AdError].
 */
fun Adapter.loadNativeAdMapperWithFailure(
  configuration: MediationNativeAdConfiguration,
  callback: MediationAdLoadCallback<NativeAdMapper, MediationNativeAdCallback>,
  expectedAdError: AdError,
) {

  this.loadNativeAdMapper(configuration, callback)

  verify(callback).onFailure(argThat(AdErrorMatcher(expectedAdError)))
}

/**
 * Calls [RtbAdapter.loadRtbNativeAdMapper] with the given [MediationNativeAdConfiguration] and
 * verifies [MediationAdLoadCallback.onFailure] with the expected [AdError].
 */
fun RtbAdapter.loadRtbNativeAdMapperWithFailure(
  configuration: MediationNativeAdConfiguration,
  callback: MediationAdLoadCallback<NativeAdMapper, MediationNativeAdCallback>,
  expectedAdError: AdError,
) {

  this.loadRtbNativeAdMapper(configuration, callback)
